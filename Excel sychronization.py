...
class ExcelVersionControl:
    def __init__(self, file1_path, file2_path, vc_path, backup_folder='backups'):
    ...
    def _create_backup(self, file_path):
        ...
        backup_path = os.path.join(self.backup_folder, f"{timestamp}_{filename}")
        shutil.copy2(file_path, backup_path)
        return backup_path
        
    def _are_rows_equal(self, row1, row2):
        """Compare two rows considering NaN as equal"""
...
if __name__ == "__main__":
    print("Starting Petheros synchronization...")
    
    # Initialize version control system
    vc_system = ExcelVersionControl(
        file1_path="TEST1.xlsx",
        file2_path="TEST2.xlsx",
        vc_path="VERSION_CONTROL.xlsx",
        backup_folder="version_backups"
    )
    
    # Perform synchronization
    print("Analyzing differences and synchronizing files...")
    changes, conflicts = vc_system.sync_files()
    
    # Print results
    vc_system.print_changes()
    
    # Generate and save report
    report = vc_system.generate_report()
    with open("synchronization_report.txt", "w") as f:
        f.write(report)
    print("\nReport saved to 'synchronization_report.txt'")